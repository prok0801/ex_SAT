...
def at_most_k(var, k):
    ...
    # (8): At most k condition (If i-th bit is true, R[i-1][k] = 0)
...
def test_at_most_k():
    ...
    id_variable = 1 
    num_vars = 4  
    variables = [0] + [i for i in range(1, num_vars + 1)]  # 1-based indexing
    ...
    for label, k in test_cases:
        print(f"\nTesting with k = {k} ({label} case)")
        at_most_k(variables, k)
        simulate_test_case(variables, k)

def simulate_test_case(variables, k):
    assigned_vars = [0] + [1 if i <= k else random.choice([0, 1]) for i in range(1, len(variables))]
    assigned_vars = assigned_vars[:k+1] + [0] * (len(variables) - k - 1)
    random.shuffle(assigned_vars[1:]) 
    print(f"Variable assignments for at most k={k}: {assigned_vars[1:]}")
